...
string deck();
string dCard (); // Drawing Cards
string newCard ();
bool aDrawn (string card);
void tPoints ();
...
string newCard () {
    ...
    unsigned short n = 0; // the number of cards drawn
    string card = dCard();
    string drawnC [52];
    while (aDrawn(card) == true) {
        card = dCard();
    }
    drawnC[n] = card;
    n++;
...
void tPoints () {
    ...
    // The cards from 2 through 9 are valued at their face value
    } else if (dCard() == face[8]) {

        points += 9;

    } else if (dCard() == face[7]) {

        points += 8;

    } else if (dCard() == face[6]) {

        points += 7;

    } else if (dCard() == face[5]) {

        points += 6;

    } else if (dCard() == face[4]) {

        points += 5;

    } else if (dCard() == face[3]) {

        points += 4;

    } else if (dCard() == face[2]) {
