...
void generateDeck(string [], int);
void shuffleDeck(string [], int);
...
void generateDeck(string deck[], int size) {
    string suits[] = {"♠", "♣", "♥", "♦"};
    ...
    for (int suit = 0; suit < 4; suit++) {
        for (int face = 0; face < 13; ++face) {
            deck[index] = faces[face] + suits[suit];
            index++;
        }
    }
}
